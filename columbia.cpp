...
void calculateTable(int row,int column){
    ...
    while(!pq.empty()){
        ...
        if(urow-1>=0&&ds[urow-1][ucolumn]>ds[urow][ucolumn]+table[urow-1][ucolumn]){
            ds[urow-1][ucolumn] = ds[urow][ucolumn]+table[urow-1][ucolumn];
             pq.push(make_pair(ds[urow-1][ucolumn],make_pair(urow-1,ucolumn)));
        }
        if(ucolumn-1>=0&&ds[urow][ucolumn-1]>ds[urow][ucolumn]+table[urow][ucolumn-1]){
            ds[urow][ucolumn-1] = ds[urow][ucolumn]+table[urow][ucolumn-1];
             pq.push(make_pair(ds[urow][ucolumn-1],make_pair(urow,ucolumn-1)));
        }
        if(urow+1<rowww&&ds[urow+1][ucolumn]>ds[urow][ucolumn]+table[urow+1][ucolumn]){
            ds[urow+1][ucolumn] = ds[urow][ucolumn]+table[urow+1][ucolumn];
             pq.push(make_pair(ds[urow+1][ucolumn],make_pair(urow+1,ucolumn)));
        }
        if(ucolumn+1<columnnn&&ds[urow][ucolumn+1]>ds[urow][ucolumn]+table[urow][ucolumn+1]){
            ds[urow][ucolumn+1] = ds[urow][ucolumn]+table[urow][ucolumn+1];
             pq.push(make_pair(ds[urow][ucolumn+1],make_pair(urow,ucolumn+1)));
        }
    }
...
int main(){
    cin >> rowww >> columnnn;
    table = vector<vector<int>>(rowww,vector<int>(columnnn,0));
